...
func (s *Server) handleCreateTrader(c *gin.Context) {
	...
	// Validate and assign strategy
	strategyID := req.StrategyID
	if strategyID == "" {
		// Try to get user's active strategy or default strategy
		activeStrategy, err := s.store.Strategy().GetActiveOrDefault(userID)
		if err != nil || activeStrategy == nil {
			c.JSON(http.StatusBadRequest, gin.H{
				"error": "No strategy specified and no active/default strategy found. Please create a strategy in Strategy Studio first.",
			})
			return
		}
		strategyID = activeStrategy.ID
		logger.Infof("✓ Auto-assigned active strategy: %s (ID: %s)", activeStrategy.Name, strategyID)
	} else {
		// Verify strategy exists and belongs to user
		strategy, err := s.store.Strategy().Get(userID, strategyID)
		if err != nil || strategy == nil {
			c.JSON(http.StatusBadRequest, gin.H{
				"error": fmt.Sprintf("Strategy does not exist or no access permission. Please select a valid strategy."),
			})
			return
		}
		logger.Infof("✓ Using selected strategy: %s (ID: %s)", strategy.Name, strategyID)
	}
	...
	traderRecord := &store.Trader{
		...
		StrategyID:           strategyID,    // Associated strategy ID (validated above)
